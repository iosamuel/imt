...
// Client instance..
export default class TMIClient extends Commands, EventEmitter {
  ...
  log = this.opts.logger || logger

  api = api

  constructor(opts: ClientOptions): void {
    super();
...
// Handle parsed chat server message..
    ...
    // Messages with "tmi.twitch.tv" as a prefix..
      ...
      switch (message.command) {
        ...
        case "USERNOTICE":
          ...
          var planName =
            _.unescapeIRC(_.get(message.tags["msg-param-sub-plan-name"], "")) ||
            null;
        ...
        case "PRIVMSG":
          ...
          if (message.tags.username === "jtv") {
            ...
            if (actionMessage) {
              ...
              this.log.info(
                `[${channel}] *<${message.tags.username}>: ${actionMessage[1]}`
              );
              this.emits(
                ["action", "message"],
  ...
  async _openConnection () {
    ...
    headers.set("Sec-WebSocket-Protocol", "irc");
    this.ws = await connectWebSocket(
      `${this.secure ? "wss" : "ws"}://${this.server}:${this.port}/`,
      headers
    );

    Reflect.apply(this._onOpen, this, []);

    try {
      for await (const data of this.ws) {
        if (typeof data === "string") {
          Reflect.apply(this._onMessage, this, [{ data }]);
        } else if (isWebSocketCloseEvent(data)) {
          Reflect.apply(this._onClose, this, []);
        }
      }
    } catch (error) {
      Reflect.apply(this._onError, this, [{ error }]);
    }
  };
    ...
    return new Promise((resolve, reject) => {
      ...
      Object.keys(this.emotesets).forEach(id =>
        this.emotesets[id].forEach(emote => {
          var emoteFunc = _.isRegex(emote.code)
            ? parse.emoteRegex
            : parse.emoteString;
  ...
  _updateEmoteset(sets) {
    this.emotes = sets;

    this._getToken()
      .then(token => {
        const headers = new Headers();
        headers.set("Authorization", `OAuth ${_.token(token)}`);
